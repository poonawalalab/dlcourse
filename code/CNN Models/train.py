...
from data import ImageDataset, BasicDataLoader
...
if __name__ == "__main__":
    ...
    all_val_inputs = torch.stack([dataset[i][0] for i in range(len(dataset))]).to(device)
    all_val_predictions = model(all_val_inputs)
    all_val_targets = torch.cat([dataset[i][1] for i in range(len(dataset))])
